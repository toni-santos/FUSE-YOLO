...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False, string=None):
        ...
        if string:
            if string.startswith("backbone"):
                backbone_idx = int(string.split('_')[1])
                m_ = self.backbone[backbone_idx]
                savelist = self.backbone_save
            elif string.startswith("fuse"):
                fuse_idx = int(string.split('_')[1])
                m_ = [self._fuse[fuse_idx]]
                savelist = self.fuse_save
            elif string.startswith("head"):
                y = x
        ...
        for m in m_:                
            ...
            x = m(x)  # run
            # NOTE: when running on fusion mode, the head needs to receive the fused features, these occupy len(self.fuse_steps) positions
            # in the savelist, so we need to shift the savelist by that much in order to get the correct indices to compare with m.i
            if string == "head":
                y.append(x if m.i in [r - len(self.fuse_steps) for r in savelist] else None)
            else:
                y.append(x if m.i in savelist else None)  # save output
            if visualize:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None, fusion=False):
...
def parse_model(d, ch=None, string=None, fuse_steps=[]):
    ...
    for i, (f, n, m, args) in enumerate(d_):  # from, number, module, args
        ...
        if ((i in fuse_steps) and (string == "backbone")) or (string == "fuse"):
            final_ch.append(c2)
        ...
        if string == None:
            ...
            save.extend(x for x in ([f] if isinstance(f, int) else f) if x not in range(-1, len(d["fuse_steps"])))  # append to savelist
        
        layers.append(m_)
        if i == 0 and (string == "backbone" or string == None):
            ch = []
        ch.append(c2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fusion", action="store_true", help="fusion model")
